...
void ExchangeBrowser::on_btnConvertVCoin_clicked()
{
    double vcoinQty = ui->txtConvertVCoinQty->text().toDouble();
    double totalBtc = _bittrexMarketSummary->getLastCurrent(double()) * vcoinQty;
    double totalUsd = totalBtc * _dBtcPriceCurrent;

    ui->lblConvertVCoinResults->setText("$" + QString::number(totalUsd, 'f', 2) +
...
void ExchangeBrowser::coinbasePrice(QNetworkReply* response)
{
    ...
    _dVcnPriceLast = _dBtcPriceCurrent * _bittrexMarketSummary->getLastCurrent(double());
}
